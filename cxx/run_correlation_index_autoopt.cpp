...
void run_workload(QueryEngine<DIM>& engine,
    ...
    size_t num_queries = 100; //workload.size();
...
int main(int argc, char** argv) {
    ...
    if (indexer->Init(data.begin(), data.end())) {
        std::cout << "Data was modified...continuing..." << std::endl;
    }
    ...
    std::cout << "Optimizing correlated dimensions with Cortex..." << std::endl;
    auto before_opt = std::chrono::high_resolution_clock::now();
    std::vector<size_t> indexes(data.size());
    std::iota(indexes.begin(), indexes.end(), 0);
    indexer->DummyInsert(data, indexes);
    auto after_opt = std::chrono::high_resolution_clock::now();
    auto tt_opt = std::chrono::duration_cast<std::chrono::nanoseconds>(after_opt - before_opt).count();
    std::cout << "Time to optimize with Cortex: " << tt_opt / 1e9 << "s" << std::endl;
