...
std::map<std::string, std::string> ParseFlags(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        std::string val = arg.substr(equals+1);   
...
std::string GetWithDefault(std::map<std::string, std::string> m,
...
std::string GetRequired(std::map<std::string, std::string> m, const std::string& key) {
    auto it = m.find(key);
    if (it == m.end()) {
...
std::vector<std::string> GetCommaSeparated(
